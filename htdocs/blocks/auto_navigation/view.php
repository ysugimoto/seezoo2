...
if ( ! function_exists('_build_autonavigation_format'))
{
	function _build_autonavigation_format($c, $data, $recursive = FALSE)
	{
			...
			if (!$recursive)
			{
				$out = array('<ul class="' . $c->get_display_mode_class() . ((!empty($c->handle_class) ? ' ' . prep_str($c->handle_class) : '')) . '">');
			}
			else
			{
				$out = array('<ul>');
			...
			foreach ($data as $key => $v)
			{
				...
				{
					$out[]        = '<li>';
					$active_class = ($v['page']['page_id'] == $CI->page_id) ? ' class="' . prep_str($c->current_class) . '"' : '';
					$exp          = explode('/', trim($v['page']['page_path'], '/'));
					$page_class   = array(end($exp));
					$target_blank = ( (int)$v['page']['target_blank'] > 0 )
					                  ? ' target="_blank"'
					                     : '';
					if ( !empty($active_class) )
					{
						$page_class[] = prep_str($c->current_class);
					}
					...
					if ( $v['page']['is_ssl_page'] > 0 )
					{
						$out[] = '<a href="' .ssl_page_link($v['page']['page_path']) . '"'
						           . $target_blank . $link_class . '>' . prep_str($v['page']['page_title']) . '</a>';
					}
					else if ( ! empty($v['page']['external_link']) )
					{
						$out[] = '<a href="' . $v['page']['external_link'] . '"' . $target_blank . '>'
						          . prep_str($v['page']['page_title']) . '</a>';
					}
					else 
					{
			...
			$out[] = '</ul>';
			if ( $c->get_display_mode_class() === 'h_nav' )
			{
				$out[] = '<br class="sz_clear" />';
			}
...
if ( ! function_exists('_build_autonavigation_breadcrumb'))
{
	function _build_autonavigation_breadcrumb($c, $data)
	{
		...
		foreach ($data as $key => $v)
		{
			if ( $v['navigation_show'] < 1 ) {
				continue;
			}
			if (is_permission_allowed($v['allow_access_user'], $CI->user_id))
			{
				$active_class = ($v['page_id'] == $CI->page_id) ? ' class="' . prep_str($c->current_class) . '"' : '';
				...
				if ($key == count($data) - 1)
				{
					$out[] = '<li' . $active_class . '>' . prep_str($v['page_title']) . '</li>';
				}
				else
				{
					if ( $v['is_ssl_page'] > 0 )
					{
						$out[] = '<li><a href="' . ssl_page_link($v['page_path']) . '"' . $link_class . '>' . prep_str($v['page_title']) . '</a></li>';
					}
					else 
					{
						$out[] = '<li><a href="' . page_link($v['page_path']) . '"' . $link_class . '>' . prep_str($v['page_title']) . '</a></li>';
					}
				}
