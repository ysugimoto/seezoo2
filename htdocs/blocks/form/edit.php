...
<ul id="sz_form_tab" class="clearfix">
	<li><a href="#content1">質問の追加</a></li>
	<li><a href="#content4">質問の編集</a></li>
	<li><a href="#content2" class="active">配置順設定</a></li>
...
<div id="content1" class="tabcontents init_hide">
...
-->
			...
			<tr>
				<td colspan ="2">
					<p class="submit">
						<input type="hidden" name="question_key" value="<?php echo $controller->question_key?>" id="q_key" />
...
<div id="content2" class="tabcontents">
	<ul id="form_preview">
		<?php foreach ($controller->edit_forms() as $value):?>
		<li class="sz_form_list" question_id="<?php echo $value->question_id;?>">
			<span><?php echo $value->question_name;?></span>
			<p class="sz_form_list_edit"><a href="javascript:void(0)" class="q_edit" qid="<?php echo $value->question_id;?>">編集</a></p>
			<p class="sz_form_list_delete"><a href="javascript:void(0)" class="q_delete" qid="<?php echo $value->question_id;?>">削除</a></p>
			<p class="sz_form_list_control"><a href="javascript:void(0)" class="fc_up">▲</a><br /><a href="javascript:void(0)" class="fc_down">▼</a></p>
		</li>
		<?php endforeach;?>
		
	</ul>
	<p class="submit" id="sort_wrapper">
		<input type="button" id="sort" value="配置順を保存する" />
	</p>
</div>
<div id="content3" class="tabcontents init_hide">
	<table class="add_forms">
		<tbody>
			<tr>
				<th>フォームのタイトル</th>
				<td><?php echo form_input(array('name' => 'form_title', 'value' => $controller->form_title));?></td>
			</tr>
			<tr>
				<th>フォームに付けるclass名</th>
				<td><?php echo form_input(array('name' => 'form_class_name', 'value' => $controller->form_class_name, 'size' => 50));?></td>
			</tr>
			<tr>
				<th>送信完了時のメッセージ<br />(HTML有効)</th>
				<td>
					<?php echo form_textarea(array('name' => 'thanks_msg', 'value' => $controller->thanks_msg, 'rows' => 3, 'cols' => 50));?>
				</td>
			</tr>
			<tr>
				<th>自動返信メールの設定</th>
				<td><input type="checkbox" value="1" name="is_remail" id="is_remail" <?php if($controller->is_remail == 1)echo 'checked="checked" ';?>/>&nbsp;問い合わせの通知を受ける
					<p id="remail" <?php if ($controller->is_remail == 0) { echo ' style="display:none;"';}?>>
						<?php echo form_input(array('name' => 're_mail', 'value' => $controller->re_mail, 'class' => 'ime-dis', 'size' => '50'))?><br />のアドレスにメールで通知する
					</p>
				</td>
			</tr>
			<tr>
				<th>画像認証（CAPTCHA）設定</th>
				<td><input type="checkbox" name="use_captcha" value="1"<?php if($controller->use_captcha == 1)echo ' checked="checked"';?> />&nbsp;画像認証を有効にする </td>
			</tr>
		</tbody>
	</table>
</div>
<div id="content5" class="tabcontents init_hide">
	...
	<p>
		送信メールタイトル：
		<?php echo form_input(array('name' => 'auto_reply_mail_subject', 'class' => 'auto_reply_mail_subject', 'value' => $controller->auto_reply_mail_subject));?>
	</p>
	
	<p>
		送信メール本文：
		<?php echo form_textarea(array('name' => 'auto_reply_mailbody', 'class' => 'auto_reply_mailbody', 'value' => $controller->auto_reply_mailbody));?>
	</p>
</div>
...
-->
			...
			<tr>
				<td colspan ="2">
					<p class="submit">
						<input type="hidden" name="question_key_edit" value="<?php echo $controller->question_key?>" />
